...
vec2 parallaxOcclusionMap()
{
	...
	float currentMapDepth = 1.0 - texture2D(displaceSampler, currentCoord).r; //current depth in the displacement map
	while (currentDepth < currentMapDepth) //gets steep parallax mapping texcoords
	{
		currentCoord -= shiftCoord; //shift coordinates
		float currentMapDepth = 1.0 - texture2D(displaceSampler, currentCoord).r; //updates mapDepth value
		currentDepth += layerDepth; //next layer
	}

	//linear interpolation
	vec2 prevCoord = currentCoord + shiftCoord; //shifts back up one layer
	float firstDepth = 1.0 - texture2D(displaceSampler, prevCoord).r - currentDepth + layerDepth; //depth value before hitting target point relative to ITS sample layer (hence adding one layerDepth)
	...
	/*
	vec2 coord = vf_texcoord + view.xy / view.z * (heightWeight * texture2D(displaceSampler, vf_texcoord).r);
	return coord;*/
}
...
void main()
{
	...
	vsNormal = normalize(vf_vsTBN * (texture2D(normalSampler, texcoord).rgb * 2.0 - vec3(1.0))); //normal mapping
